...
/**
* 
* @package MDWS Application
* @author Patrick McGuire <alerts@mdwestserve.com>
* @author Paolo Frias <intern@mdwestserve.com>
* @subpackage Legacy
*/  
...
/* test and append % to the barcode */
$pos = strpos($barcode, '%');
if ($pos === false) {
	$barcode .= '%';
}
...
if (empty ($width)) $width = 350;
if (empty ($height)) $height = 60;
if (empty ($barcode)) $barcode = 'MISSING ID';
...
* @param string $text
* @method Barcode39(string $barcode, int $width, int $height, string $quality, string $format, string $text)
*/
...
* 
* @param string $im image file
* @param string $format image format
* @param string $quality image quality
* @method hasRush(string $im, string $format, string $quality)
*/
...
* 
* @param string $Asc character
* @method hasRush(string $Asc)
* @return string binary for respective ASCII
*/
function Code39 ($Asc)
{
        switch ($Asc)
